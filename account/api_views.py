...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def passwordchangerequest(self,request):
        ...
        password_reset_url = request.build_absolute_uri(
            reverse('account-passwordchangeconfirm',kwargs={'uidb64':uidb64,'token':token})
        )
    ...
    @action(detail=False,methods=['POST'],url_path='passwordchangeconfirm/(?P<uidb64>[^/.]+)/(?P<token>[^/.]+)')
