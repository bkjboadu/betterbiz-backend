...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def get_user_details(self, request):
        ...
        try:
            ...
            if user_id:
                ...
                return Response(
                    {"message": "Invalid token", "result": False},
            ...
            print(e)
            return Response(
                {"message": "Token decoding error", "result": False},
    ...
    def signup(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                if email_sent == True:
                    return Response(
                        {"message": "Email verification link sent", "result": True},
                        status=status.HTTP_201_CREATED,
                    )
                else:
                    return Response(
                        {
                            "message": f"Error sending email {email_sent}",
                            "result": False,
                        },
    ...
    def getverificationlink(self, request, pk=None):
        ...
        try:
            ...
            if email_sent == True:
                ...
                return Response(
                    {"message": f"Error sending email {email_sent}", "result": False},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
        except Exception as e:
            return Response(
                {"message": str(e), "result": False},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def passwordchangerequest(self, request):
        ...
        return Response({"message": "Password reset link sent", "result": True})
    ...
    def passwordchangeconfirm(self, request, uidb64, token):
        ...
        return Response({"message": message, "result": True})
    ...
    def passwordreset(self, request, pk=None):
        ...
        if new_password != confirm_new_password:
            return Response({"message": "New password do not match"})

        # if all checks are okay, set the new password
        user.set_password(new_password)
        user.save()
        return Response({"message": "Password updated successfully", "result": True})
    ...
    def edit_profile(self, request, pk=None):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": serializer.data, "result": True})
        return Response(
            {"message": serializer.errors, "result": False},
            status=status.HTTP_400_BAD_REQUEST,
        )

    @action(detail=True, methods=["POST"])
    def setdefaultcompany(self, request, pk=None):
        user = self.get_object()
        if not user:
            return Response(
                {"message": "User not found", "result": False},
        ...
        try:
            ...
            if business_id is not None:
                ...
                user.save()
                return Response(
                    {"message": "User default company changed", "result": True}
                )
        except:
